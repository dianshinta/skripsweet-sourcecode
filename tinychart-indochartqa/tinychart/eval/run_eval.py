...
if __name__ == '__main__':
    ...
    for result_file in result_files:
        ...
        # ---------------------------------------------------------
        # Detect whether this dataset is Program-of-Thought (PoT)
        # based on the instruction inside the question.
        # ---------------------------------------------------------
        is_pot_dataset = False
        if len(result_data) > 0:
            question = result_data[0].get("question", "").lower()
            if "jawab menggunakan langkah perhitungannya" in question:
                is_pot_dataset = True
        ...
        # elif (
        #     'chartqagptpot-' in dataset_name
        #     or 'chartqatemplatepot-' in dataset_name
        #     or is_pot_dataset
        # ):
        ...
        # =====================================================
        # Custom evaluation for Indonesian ChartQA dataset
        # =====================================================
        
        if is_pot_dataset:
            # Program-of-Thought evaluation
            pot_result, pot_acc, error_rate = chartqapot_evaluator(result_data)
            write_jsonl(pot_result, file)
            dataset2metric[dataset_name] = round(pot_acc * 100, 2)
            print(f'PoT Accuracy: {pot_acc}')
    ...
    #     write_jsonl(
    #         oracle_results,
    #         os.path.join(args.input, 'merged-oracle.jsonl')
    #     )
    ...
    #     write_jsonl(
    #         rule_results,
    #         os.path.join(args.input, 'merged-rule.jsonl')
    #     )
